...
export default {
  metaInfo: {
    ...
    meta: [
      { charset: 'utf-8' },
      { name: 'description', content: 'Hi! My name is Vladimir and I am a teapot.' },
  ...
  beforeMount() {
    if (this.$isMobile()) {
      if (Math.random() < 0.5)
        this.shaderProgram = '/shaders/slime.glsl'
      else
        this.shaderProgram = '/shaders/spirits.glsl'
    }
    else {
      if (Math.random() < 0.25)
        this.shaderProgram = '/shaders/spirits.glsl'
      else if (Math.random() < 0.50)
        this.shaderProgram = '/shaders/silk.glsl'
      else if (Math.random() < 0.75)
        this.shaderProgram = '/shaders/colors.glsl'
      else 
        this.shaderProgram = '/shaders/slime.glsl'
    }
  },
